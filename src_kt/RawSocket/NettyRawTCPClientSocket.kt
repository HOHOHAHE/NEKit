...
class NettyRawTCPClientSocket(
    ...
    override fun write(data: ByteArray) {
        ...
        if (currentChannel == null || !currentChannel.isActive) {
            ...
            delegate?.get()?.didErrorOccur(IOException("Socket not connected or channel is null."), this@NettyRawTCPClientSocket)
            return
        }
        ...
        future.addListener { f ->
            if (f.isSuccess) {
                ...
                delegate?.get()?.didWrite(data, this@NettyRawTCPClientSocket)
            } else {
                logger.error("Failed to write {} bytes to Netty channel {}: {}", data.size, currentChannel, f.cause().message, f.cause())
                delegate?.get()?.didErrorOccur(f.cause(), this@NettyRawTCPClientSocket)
            }
        }
    }
    ...
    override fun readData() {
        logger.warn("readData() called on NettyRawTCPClientSocket. Netty reads are event-driven and handled automatically by channelRead.")
    ...
    override fun readDataTo(length: Int) {
        logger.warn("readDataTo(length={}) called on NettyRawTCPClientSocket. Netty reads are event-driven and handled automatically by channelRead.", length)
        // Netty handles reading automatically through channelRead events
        // This method is a no-op for Netty implementation
    }

    override fun readDataTo(data: ByteArray) {
        logger.warn("readDataTo(data.size={}) called on NettyRawTCPClientSocket. Netty reads are event-driven and handled automatically by channelRead.", data.size)
        // Netty handles reading automatically through channelRead events
        // This method is a no-op for Netty implementation
    }

    override fun readDataTo(data: ByteArray, maxLength: Int) {
        logger.warn("readDataTo(data.size={}, maxLength={}) called on NettyRawTCPClientSocket. Netty reads are event-driven and handled automatically by channelRead.", data.size, maxLength)
        // Netty handles reading automatically through channelRead events
        // This method is a no-op for Netty implementation
    }
