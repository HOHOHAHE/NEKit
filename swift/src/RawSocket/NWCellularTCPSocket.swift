...
public class NWCellularTCPSocket: NSObject, RawTCPSocketProtocol {
    ...
    /**
     Connect to remote host using cellular-only path.
    ...
    public func connectTo(host: String, port: Int, enableTLS: Bool, tlsSettings: [AnyHashable: Any]?) throws {
        ...
        // Build NWParameters and tune TCP for better latency
        let parameters: NWParameters = enableTLS ? NWParameters(tls: NWProtocolTLS.Options()) : NWParameters.tcp
        let tcp = NWProtocolTCP.Options()
        tcp.noDelay = true
        #if swift(>=5.2)
        // Try enabling TCP Fast Open to reduce handshake latency on supported stacks
        ...
        conn.start(queue: QueueFactory.getQueue())
    }
    ...
    public func readData() {
        ...
        connection!.receive(minimumIncompleteLength: 1, maximumLength: Opt.MAXNWTCPSocketReadDataSize) { data, _, isComplete, error in
            if let error = error {
                DDLogError("NWCellularTCPSocket receive error: \(error)")
    ...
    public func readDataTo(length: Int) {
        ...
        connection!.receive(minimumIncompleteLength: length, maximumLength: length) { data, _, isComplete, error in
            if let error = error {
                DDLogError("NWCellularTCPSocket readLength error: \(error)")
                self.queueCall { self.disconnect() }
                return
            }
            if isComplete {
    ...
    // State observation replaced by NWConnection.stateUpdateHandler
    ...
    private func send(data: Data) {
        ...
        self.connection!.send(content: data, completion: .contentProcessed({ error in
            self.queueCall {
                ...
                if let error = error {
                    DDLogError("NWCellularTCPSocket write error: \(error)")
                    self.disconnect()
                    return
                }
